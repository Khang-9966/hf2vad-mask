...
def samples_extraction(dataset_root, dataset_name, mode, all_bboxes, save_dir):
    ...
    # frames dataset
    mask_dataset = get_dataset(
        dataset_name=dataset_name,
        dir=os.path.join(dataset_root, dataset_name),
        context_frame_num=4, mode=mode,
        border_mode="predict", all_bboxes=all_bboxes,
        patch_size=32, mask_dataset=True
    )
    # flows dataset
    ...
    chunked_samples = dict(sample_id=[], appearance=[], mask_appearance=[], motion=[], bbox=[], pred_frame=[])
    ...
    for idx in range(len(dataset)):
        ...
        mask_batch, _ = mask_dataset.__getitem__(idx)
        ...
        if len(cur_bboxes) > 0:
            ...
            mask_batch = img_batch_tensor2numpy(mask_batch)
            ...
            for idx_box in range(cur_bboxes.shape[0]):
                ...
                chunked_samples["mask_appearance"].append(mask_batch[idx_box])
                ...
                if cnt == num_samples_each_chunk:
                    ...
                    chunked_samples["mask_appearance"] = np.array(chunked_samples["mask_appearance"])
