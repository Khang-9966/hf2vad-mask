...
def get_inputs(file_addr,mask=False):
    ...
    if file_format == 'mat':
        ...
        if mask:
            return np.expand_dims(cv2.imread(file_addr,0),axis=-1)
        else:
        ...
        for i in range(len(bboxes)):
            ...
            for j in range(img.shape[0]):  # temporal patches
                ...
                if len(cur_patch.shape) == 2:
                    cur_patch = np.expand_dims(cur_patch,-1)
...
class shanghaiTech_dataset(Dataset):
    def __init__(self, dir, mode='train', context_frame_num=0, border_mode='elastic', file_format='.jpg',
                 all_bboxes=None, patch_size=32, of_dataset=False, mask_dataset=False):
        '''
...
class Chunked_sample_dataset(Dataset):
    def __init__(self, chunk_file, last_flow=False, transform=transform):
        ...
        self.chunked_samples_mask_appearance = self.chunked_samples["mask_appearance"]        
    ...
    def __getitem__(self, indice):
        ...
        mask_appearance = self.chunked_samples_mask_appearance[indice]        
        ...
        return self.transform(x), self.transform(y),self.transform(mask_x), \
